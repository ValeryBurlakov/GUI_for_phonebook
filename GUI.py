...
from tkinter import DISABLED, END, TOP  # явное импортирование
...
def print_phone_book():
    ...
    #     print(data_list[index], index + 1)
    # choose=lbox.curselection()
    # messagebox.showinfo('ВЫХОД', f'{choose}')
...
def click_button_new():
    ...
    add_contact.config(state=tk.NORMAL, command=new_contact)
...
def click_button_del():
    ...
    add_contact.config(state=tk.NORMAL, command=delete_contact)
...
def click_button_change():
    logging.info('нажали кнопку поиск контакта')
    add_contact.config(text="Изменить")
    # entry_name_label.config(text="имя новой папки")
    enter_name.config(state=tk.NORMAL, command=name_input)
    enter_surname.config(state=tk.DISABLED, command=surname_input)
    enter_phone.config(state=tk.DISABLED, command=iphone_input)
    enter_email.config(state=tk.DISABLED, command=iemail_input)
    add_contact.config(state=tk.NORMAL, command=change_details)
...
def new_contact():
    ...
    def added_contact():
        ...
        with open('BD.json', 'w', encoding='utf-8') as outfile:
            ...
            logging.info('Added contact successful')
    ...
    if os.path.exists("BD.json"):
            ...
            if len(t) > 0:
                ...
                if count_fullname > 0:
                    ...
                    messagebox.showinfo('ОШИБКА ФИО', f'такое фио уже есть, добавьте новый контакт(,\nлибо измените '
                                                      f'имеющийся в соответствующем меню')
                else:
                    if count_phone > 0:
        ...
        with open('BD.json', 'w', encoding='utf-8') as fh:
            ...
            fh.write(json.dumps(BD,
                                ensure_ascii=False))  # преобразовываем словарь data в
            # unicode-строку и записываем в файл
...
def change_details():
    ...
    for i in data["phone_book"]:
        if i["name"] == iname and i["surname"] == isurname:
            ...
            email = i["E-mail"]
            surname = i["surname"]
            print("\033[1m какие данный хотите изменить?\033[0m(ФИО, номер, почту): ")
            ...
            if answer == 0:
                print("\033[1m выходим в меню\033[0m")
            if answer == 1:
                answer_name = int(input("1 - имя, 2 - фамилию: "))
                if answer_name == 1:
                    change_name = input(f"\033[1m Меняем имя {iname} на:\033[0m ")
                    i["name"] = change_name
...
def substring_search():
    ...
    with open('BD.json', encoding='utf8') as openfile:
        ...
        if len(names_list_index) == 0:
            ...
            id_search = int(input("\033[1mВведите\033[0m \033[32mID\033[0m \033[1mконтакта для вывода подробной "
                                  "информации о нём,\033[0m \033[32m0\033[0m - \033[1mвыход в меню:\033[0m "))
            found = 0
            if id_search == 0:
                print("\033[1m выходим в меню\033[0m")
            ...
            for i in t:
                if i["id"] == id_search:
                    print(f'\033[32m name: \033[0m{i["name"]} {i["surname"]} \n' +
                          f'\033[32m phone: \033[0m {i["phone"]} \n' +
                          f'\033[32m E-mail: \033[0m {i["E-mail"]}')
                    logging.info('full output')
                    found += 1
                    return found
            if found == 0:
                # print(f"\033[1mКонтакт с id\033[0m - \033[32m{id_search}\033[0m \033[1m отсутствует в
                # телефонной книге!\033[0m]")
...
def copy():
    ...
    with open('BD.json', 'r') as f:  # открыли файл с данными
        ...
        with open(n, 'w', encoding='utf-8') as openfile:  # Открываем файл
            ...
            add_contact.config(text="операция")
            logging.info('Copying successful')
